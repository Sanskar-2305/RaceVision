...
RV_BG = (8, 10, 14)
RV_PANEL = (15, 18, 24, 235)
RV_PANEL_LIGHT = (22, 26, 34, 240)

RV_WHITE = (245, 247, 250)
RV_TEXT = (190, 196, 205)
RV_MUTED = (115, 123, 135)

RV_RED = (225, 6, 0)
RV_RED_DARK = (130, 10, 8)
...
class WeatherComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        arcade.draw_rect_filled(panel_rect, RV_PANEL)

        arcade.draw_rect_outline(panel_rect, RV_BORDER, 1)
...
class LeaderboardComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        # Keep enough space for all drivers
        panel_height = 30 + len(self.entries) * self.row_height + 35
        panel_height = min(panel_height, window.height - 70)

        panel_center_y = window.height - 20 - panel_height / 2

        # panel_rect = arcade.XYWH(
        #     self.x + self.width / 2, panel_center_y, self.width, panel_height
        # )
        ...
        for i, (code, color, pos, progress_m) in enumerate(new_entries):
            ...
            if code in self.selected:
                ...
                # Selected driver = WHITE text
                text_color = RV_WHITE
            else:
                # Normal driver = team color
                text_color = color

            # ALWAYS draw the driver name
...
class SessionInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        # ============================================================
        # RACEVISION SESSION BANNER
        # ============================================================

        banner_height = 68
        banner_width = min(950, window.width - 40)
        ...
        if circuit and circuit != event:
            title_parts.append(circuit)
        title = "RACEVISION  |  " + "  •  ".join(title_parts)

        self._text.font_size = 18
        self._text.bold = True
        self._text.color = RV_WHITE
        self._text.text = title

        self._text.x = center_x
        self._text.y = top_y - 21
        ...
        if total_laps:
            info_parts.append(f"{total_laps} Laps")

        info_line = "   |   ".join(info_parts)

        self._text.font_size = 12
        self._text.bold = False
        self._text.color = RV_MUTED
        self._text.text = info_line

        self._text.x = center_x
        self._text.y = top_y - 47
        self._text.anchor_x = "center"
        self._text.anchor_y = "center"
