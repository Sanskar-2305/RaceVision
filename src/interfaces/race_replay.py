...
class F1RaceReplayWindow(arcade.Window):
    def __init__(
        ...
        super().__init__(
            SCREEN_WIDTH,
            SCREEN_HEIGHT,
            "RaceVision | F1 Telemetry Replay",
            resizable=True,
        )
        self.maximize()
        ...
        self._precomputed_status_laps = self._compute_status_laps(
            frames, track_statuses
        )
        ...
        arcade.set_background_color((5, 7, 12))
        ...
        if session is not None and hasattr(session, "laps"):
            try:
                ...
                for _, row in session.laps.iterrows():
                    ...
                    lap = int(lap_num)
                    time_s = (
                        lap_time_td.total_seconds() if pd.notna(lap_time_td) else -1.0
                    )
                    lap_end_td = row.get("Time")
                    end_time_s = (
                        lap_end_td.total_seconds() if pd.notna(lap_end_td) else None
                    )
                    line_end_td = row.get("Sector3SessionTime")
                    line_end_s = (
                        line_end_td.total_seconds()
                        if pd.notna(line_end_td)
                        else end_time_s
                    )
                    ...
                    is_pit_entry = False
                    if "PitInTime" in session.laps.columns and pd.notna(
                        row.get("PitInTime")
                    ):
                        is_pit_entry = True
                    is_out_lap = False
                    if "PitOutTime" in session.laps.columns and pd.notna(
                        row.get("PitOutTime")
                    ):
                        is_out_lap = True
                ...
                if result:
                    ...
                    for code, entries in result.items():
                        ...
                        for entry in entries:
                            ...
                            if fb_entry is None:
                                continue
                            if (
                                entry.get("time_s", -1.0) <= 0
                                and fb_entry.get("time_s", -1.0) > 0
                            ):
                                entry["time_s"] = fb_entry["time_s"]
                                entry["time_source"] = "frame_backfill"
                                entry["source"] = "derived"
                                if (
                                    entry.get("end_time_s") is None
                                    and fb_entry.get("end_time_s") is not None
                                ):
                                    entry["end_time_s"] = fb_entry["end_time_s"]
                                if (
                                    entry.get("line_time_s") is None
                                    and fb_entry.get("end_time_s") is not None
                                ):
                            ...
                            if (
                                fb_entry.get("is_pit_affected")
                                or fb_entry.get("is_pit")
                            ) and not entry.get("is_pit_affected"):
                                entry["is_pit_affected"] = True
                                if (
                                    not entry.get("pit_confidence")
                                    or entry.get("pit_confidence") == "none"
                                ):
                                    entry["pit_confidence"] = fb_entry.get(
                            ...
                            if (
                                fb_entry.get("pace_baseline_s") is not None
                                and entry.get("pace_baseline_s") is None
                            ):
                    ...
                    replay_time_offset_s = (
                        F1RaceReplayWindow._estimate_official_replay_time_offset(
                            result,
                            classified_fallback,
                        )
                    )
                    ...
                    try:
                        _, stream_data, _ = ffapi._extended_timing_data(
                            session.api_path
                        )
                        ...
                        missing_stream_cols = required_stream_cols.difference(
                            stream_data.columns
                        )
                        ...
                        if (
                            ...
                            for _, sess_lap_row in (
                                session.laps[["Driver", "LapNumber", "Position"]]
                                ...
                                try:
                                    ...
                                    pos_int = (
                                        int(pos_val) if pd.notna(pos_val) else None
                                    )
                                    official_pos_lookup[
                                        (
                                            str(sess_lap_row["Driver"]),
                                            int(sess_lap_row["LapNumber"]),
                                        )
                                    ] = pos_int
                        ...
                        def _parse_timing_delta(raw_value, row_position=None):
                            ...
                            if (
                                upper_text.startswith("LAP")
                                or upper_text.endswith(" L")
                                or upper_text.endswith("L")
                            ):
                                return 0.0 if leader_row else None
                        ...
                        for code, entries in result.items():
                            ...
                            if not drv_num:
                                continue
                            drv_stream_rows = stream_data[
                                stream_data["Driver"].astype(str) == drv_num
                            ]
                            ...
                            for lap_entry in entries:
                                ...
                                if target_time_s is None:
                                    continue
                                first_time = pd.to_timedelta(
                                    float(target_time_s), unit="s"
                                )
                                official_pos = official_pos_lookup.get((code, lap_no))
                                time_delta = (
                                    drv_stream_rows["Time"] - first_time
                                ).abs()
                                local_window = drv_stream_rows[
                                    time_delta <= pd.to_timedelta(20, unit="s")
                                ]
                                matched_stream_row = None
                                if official_pos is not None and not local_window.empty:
                                    pos_rows = local_window[
                                        local_window["Position"] == official_pos
                                    ]
                                    if not pos_rows.empty:
                                        ref_idx = (
                                            (pos_rows["Time"] - first_time)
                                            .abs()
                                            .idxmin()
                                        )
                                        matched_stream_row = pos_rows.loc[ref_idx]
                                if (
                                    matched_stream_row is None
                                    and not local_window.empty
                                ):
                                    ref_idx = (
                                        (local_window["Time"] - first_time)
                                        .abs()
                                        .idxmin()
                                    )
                                    matched_stream_row = local_window.loc[ref_idx]
                                if matched_stream_row is None:
                                    candidate_rows = drv_stream_rows[
                                        drv_stream_rows["Time"] <= first_time
                                    ]
                                    if (
                                        official_pos is not None
                                        and not candidate_rows.empty
                                    ):
                                        pos_rows = candidate_rows[
                                            candidate_rows["Position"] == official_pos
                                        ]
                                        if not pos_rows.empty:
                                            ref_idx = (
                                                (pos_rows["Time"] - first_time)
                                    ...
                                    if (
                                        ...
                                        matched_stream_row = candidate_rows.sort_values(
                                            "Time"
                                        ).iloc[-1]
                                if matched_stream_row is None:
                                    ref_idx = (
                                ...
                                if target_position is None and pd.notna(
                                    matched_position
                                ):
                                    try:
                                ...
                                if (
                                    ...
                                    and not interval_quality_rows.empty
                                ):
                                    interval_quality_rows = interval_quality_rows[
                                        interval_quality_rows["Position"]
                                        == target_position
                                    ]
                                if not interval_quality_rows.empty:
                                    ...
                                    for _, cand_row in interval_quality_rows.iterrows():
                                        ...
                                        cand_dt_s = abs(
                                            float(
                                                (
                                                    cand_row["Time"] - first_time
                                                ).total_seconds()
                                            )
                                        )
                                        scored_intervals.append(
                                            (cand_dt_s, float(cand_interval_s))
                                        )
                                    if scored_intervals:
                                        scored_intervals.sort(key=lambda item: item[0])
                                        close_intervals = [
                                            val
                                            for dt_s, val in scored_intervals[:4]
                                            if dt_s <= 6.0
                                        ]
                                ...
                                gap_s = _parse_timing_delta(
                                    matched_stream_row.get("GapToLeader"),
                                    matched_position,
                                )
                                ...
                                if matched_stream_row.get(
                                    "Time"
                                ) is not None and pd.notna(
                                    matched_stream_row.get("Time")
                                ):
                                ...
                                if pd.notna(matched_position):
                                    try:
                                        lap_entry["official_stream_position"] = int(
                                            matched_position
                                        )
                                    except (TypeError, ValueError):
                                        pass
                                if interval_s is not None:
                                    lap_entry["official_interval_to_ahead_s"] = float(
                                        interval_s
                                    )
                            ...
                            for item in ordered_items:
                                ...
                                if pos == 1:
                                    ...
                                    if (
                                        ...
                                        predicted_gap_s = float(prev_gap_s) + float(
                                            item["interval_s"]
                                        )
                                ...
                                item["entry"]["official_gap_to_leader_s"] = float(
                                    assigned_gap_s
                                )
                                item["entry"]["official_gap_source"] = source
                    except Exception as e:
                        print(
                            f"Warning: official gap enrichment failed, using fallback gaps where needed: {e}"
                        )
                    try:
                        import pandas as pd
                        ...
                        if results_df is not None and not results_df.empty:
                            winner_rows = results_df[
                                results_df["ClassifiedPosition"] == "1"
                            ]
                            if not winner_rows.empty:
                                ...
                                if winner_code in result and winner_laps > 0:
                                    winner_entries = {
                                        e["lap"]: e for e in result[winner_code]
                                    }
                                ...
                                for _, res_row in results_df.iterrows():
                                    ...
                                    lap_entry = next(
                                        (
                                            e
                                            for e in result[code]
                                            if e["lap"] == winner_laps
                                        ),
                                        None,
                                    )
                            ...
                            for _, res_row in results_df.iterrows():
                                ...
                                classified_pos = str(
                                    res_row.get("ClassifiedPosition", "")
                                ).strip()
                                ...
                                if (
                                    ...
                                    completed_time_s = (
                                        ...
                                        if completed_entry.get("line_time_s")
                                        is not None
                                ...
                                if not (
                                    has_follow_on_terminal_event
                                    or has_same_lap_terminal_event
                                ):
                                    continue
                                ...
                                terminal_event_time_s = (
                                    ...
                                    if terminal_entry.get("replay_line_time_s")
                                    is not None
                                ...
                                if terminal_event_time_s is not None:
                                    terminal_entry["terminal_event_time_s"] = (
                                        terminal_event_time_s
                                    )
                                if (
                                    not status_text
                        ...
                        print(
                            f"Warning: result-status enrichment failed, continuing without terminal/result metadata: {e}"
                        )
                    return F1RaceReplayWindow._classify_lap_entries(
                        result, official_data=True
                    )
            except Exception as e:
    ...
    def _estimate_official_replay_time_offset(official_result, fallback_result):
        ...
        for code, entries in official_result.items():
            ...
            for entry in entries:
                ...
                if fb_entry is None or not isinstance(line_time_s, (int, float)):
                    continue
    ...
    @staticmethod
    def _attach_replay_aligned_lap_times(
        ...
        for code, entries in result.items():
            ...
            for entry in entries:
                if entry.get("time_source") == "frame_backfill":
                    ...
                    if fb_entry is not None and isinstance(
                        fb_entry.get("end_time_s"), (int, float)
                    ):
                        replay_end = float(fb_entry["end_time_s"])
    ...
    @staticmethod
    def _compute_fallback_lap_times_raw(
        ...
        for entries in result.values():
            ...
            for i, entry in enumerate(entries):
                ...
                entry.setdefault(
                    "is_pit_affected",
                    bool(
                        entry.get("is_pit_entry", False) or entry.get("is_pit", False)
                    ),
                )
                ...
                entry.setdefault(
                    "pit_confidence",
                    "official"
                    if entry.get("is_pit_entry") and entry.get("source") == "official"
                    else "none",
                )
                ...
                if (
                    ...
                    next_tyre_life = (
                        next_entry.get("tyre_life", -1) if next_entry else -1
                    )
                ...
                if (
                    entry.get("is_pit_entry")
                    and next_entry
                    and next_entry["lap"] == entry["lap"] + 1
                ):
    ...
    def _compute_status_laps(frames, track_statuses):
        ...
        for status in track_statuses:
            ...
            if (
                result
                and result[-1]["status"] == code
                and result[-1]["end_lap"] >= start_lap - 1
            ):
    ...
    def on_draw(self):
        ...
        if hasattr(self, "drs_zones") and self.drs_zones and self.toggle_drs_zones:
            drs_color = (80, 220, 170)  # Bright green for DRS zones
        ...
        # Draw HUD - Top Left
        if self.visible_hud:
            self.lap_text.text = f"RACEVISION  •  {lap_str}"
            self.time_text.text = (
                f"SESSION TIME  {time_str}   •   SPEED ×{self.playback_speed}"
            )
            ...
            if current_track_status == "2":
                ...
                self.status_text.color = (245, 190, 45)
            elif current_track_status == "5":
                self.status_text.text = "■  RED FLAG"
                self.status_text.color = (235, 55, 70)
            elif current_track_status == "6":
                self.status_text.text = "VSC  • VIRTUAL SAFETY CAR"
                self.status_text.color = (255, 150, 45)
            elif current_track_status == "4":
                self.status_text.text = "SC  • SAFETY CAR"
                self.status_text.color = (190, 105, 55)
